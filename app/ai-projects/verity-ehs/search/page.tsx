...
import { Card, CardContent, CardHeader, CardTitle } from "@/app/components/ehs/ui/card"
import { Badge } from "@/app/components/ehs/ui/badge"
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "@/app/components/ehs/ui/collapsible"
import { useLanguage } from "@/app/components/ehs/language-provider"
import { authHeaders } from "@/app/lib/ehs-auth"
import { getSearchHistory, addToSearchHistory, clearSearchHistory } from "@/app/lib/ehs-search-history"
...
function SearchContent() {
  ...
  async function performSearch(q: string) {
    ...
    try {
      const res = await fetch("/api/ehs/search", {
        method: "POST",
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (query.trim()) {
      setMessages([])
      router.push(`/ai-projects/verity-ehs/search?q=${encodeURIComponent(query.trim())}`)
    }
  }
                  ...
                  {searchHistory.map((item, i) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setShowHistory(false)
                        setMessages([])
                        router.push(`/ai-projects/verity-ehs/search?q=${encodeURIComponent(item)}`)
                      }}
            ...
            <div className="space-y-4 mb-4">
                                ...
                                <div className="px-4 pb-4 space-y-3">
                                  {grouped.map((citation) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        <Link href={`/ai-projects/verity-ehs/documents/${citation.document_id}`}>
              ...
              {!loading && (
                <form onSubmit={handleFollowUp} className="flex gap-2">
                  ...
                  />
                  <Button type="submit" disabled={!followUp.trim()} className="min-h-[48px] ehs-btn-primary">
